...
在看 RN 源码时候发现了 `ErrorUtils`，看代码可以设置处理处理。
...
下图是写了一个 RN Demo 工程，在 Debug Text 控件上加了事件监听代码，内部人为触发 crash
...
6. 整个 APM 的架构图如下
   ...
   - skyline 是埋点 SDK，通过 sessionId 来关联日志数据
   - wax 上面介绍过了，是一种多端项目管理的一
   - log SDK 就是数据上报 SDK。下篇文章会讲
